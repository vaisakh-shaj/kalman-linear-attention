...
@pytest.mark.parametrize("preset", [PLAIN, MAMBA, dict(value_rank="dt", var_rank="dt"),
...
def test_conv_value_is_exactly_z():
    """value_rank='conv' must pass z through untouched, not merely something shaped like it."""
...
def test_all_parameters_get_gradients(preset):
    ...
    dead = [n for n, p in lay.named_parameters()
...
def test_phi_floor_is_off_by_default():
    ...
    assert torch.equal(phi, torch.zeros_like(phi)), "phi was floored on the default path"
    phi.sum().backward()
