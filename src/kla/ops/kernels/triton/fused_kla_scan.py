"""Single fused KLA scan kernel (forward) — the CUDA-style fully fused path.
...
``[B,L,M,S]`` tensor in HBM (the per-(b,l,m,s) torch "glue" of the composed
backend is what left it ~6–11× the CUDA kernel):
...
Scope: ``decode_from_prior=False``. The backend falls back to the composed tiled
scans for prior decode. ``a``/``p`` are static ``[M, S]`` (the paper's
time-invariant dynamics), loaded once per program outside the sequence loop.
Forward only for now — training still uses the composed tiled bwd.
"""
...
def fused_kla_forward(
    ...
    ``(y, yvar)`` ``[B,L,M]`` plus the final ``(lam, eta)``.
    """
    B, L, M = msi.shape
    S = h.shape[2]
