...
//
// Floor only — matching the torch/triton reference (`phi.clamp_min(EPS)`). An
// upper clip here would corrupt the posterior rather than stabilize it: phi
// feeds the precision scan while r feeds the information vector *unclipped*, so
// saturating phi breaks the Lambda^v cancellation in mean = r/phi = v/k and
// inflates both the mean and the variance by the saturation ratio. phi enters
// the leaf only linearly (A = (1+q*phi)/a2, B = phi) and damped by q, and the
// trace-norm renormalizes after every compose, so a large phi is no threat to
// float32 here. Bound Lambda^v and k at the layer instead (obs_var_min /
// obs_var_max / qk_norm / clip_value), where the bound applies to every backend
// identically and is applied *before* the phi/r split.
__device__ __forceinline__ void compute_phi_r(
    float h, float sig_inv, float mu_sig,
    float &phi, float &r, float &raw_phi
) {
    raw_phi = h * h * sig_inv;
    phi = fmaxf(raw_phi, KLA_EPS);
    r = mu_sig * h;
}
