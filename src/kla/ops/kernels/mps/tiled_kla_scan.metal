/******************************************************************************
 * KLA Metal kernels — tiled strategy (the "triton-shaped" path)
 *   tiled_kla_scan.metal
 *
 * Time as a *parallel* axis, for the case the lane-per-state kernels cannot
 * fill: batch-1 prefill, where B*M*S alone leaves the GPU short of threads.
 *
 * One threadgroup owns one (batch, channel) pair and streams the sequence in
 * tiles of KLA_ROWS * KLA_CHUNK timesteps, carrying (lambda, eta) across tiles.
 ...
 *   KLA_CHUNK     timesteps each of those threads walks serially
 *
 ...
 * fused_kla_scan.metal, bought in exchange for KLA_ROWS times the parallelism.
 * Walking KLA_CHUNK steps serially per thread is what keeps it at 4x rather
 * than the log(tile) composes a pure Hillis-Steele scan over timesteps costs.
 *
 * Six phases per tile:
 *   A  compose this thread's KLA_CHUNK Moebius leaves    -> thread aggregate
 ...
 *
 * Forward only. Training stays on fused_kla_scan.metal, whose per-step adjoint
 * is exact and needs no composition at all.
 ******************************************************************************/
...
// negligible next to the KLA_CHUNK serial steps each element already cost.
...
kernel void kla_tiled_fwd(
    ...
    device const float *msi    [[buffer(4)]],   // v.Lambda^v [B, L, M]
    device const float *si     [[buffer(5)]],   // Lambda^v   [B, L, M]
    device const float *k      [[buffer(6)]],   // key        [B, L, S]
    device const float *qw     [[buffer(7)]],   // query      [B, L, S]
    device const float *a      [[buffer(8)]],   // decay         [M, S]
    device const float *p      [[buffer(9)]],   // process noise [M, S]
    device const float *lam0   [[buffer(10)]],  // [B, M, S]
    device const float *eta0   [[buffer(11)]],  // [B, M, S]
    constant int &L_           [[buffer(12)]],
    constant int &M_           [[buffer(13)]],
    constant int &S_           [[buffer(14)]],
    constant int &prior        [[buffer(15)]],  // decode_from_prior
    ...
    float var_h[KLA_CHUNK];
    float alpha_h[KLA_CHUNK];
    float r_h[KLA_CHUNK];

    const uint tile_len = KLA_ROWS * KLA_CHUNK;
    const uint t_base = ty * KLA_CHUNK;
