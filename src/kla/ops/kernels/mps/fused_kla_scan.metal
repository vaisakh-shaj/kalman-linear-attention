/******************************************************************************
 * KLA Metal kernels — fused strategy (the "CUDA-shaped" path)
 *   fused_kla_scan.metal
 *
 * The whole KLA scan as two kernels, forward and backward, with no
 * [B,L,M,S] intermediate ever reaching device memory:
 *
 ...
 *
 * Kernel-internal names follow the CUDA kernels rather than the paper:
 * ``msi`` is v.Lambda^v, ``si`` is Lambda^v, ``k`` the observation map and
 * ``qw`` the read-out. ``p`` is the process noise (the CUDA sources call it
 * ``q``, which collides with the query; renamed here).
 *
 ...
 *
 * Backward. Unlike the CUDA kernel's, this one is the *exact* adjoint. That
 * falls out of applying the Moebius map instead of composing it: the CUDA
 * backward has to differentiate a trace-normalized prefix product, which is
 * what makes its d(Lambda^v)/d(k)/d(a)/d(p) approximate, whereas a per-step
 * map has the elementary derivative
 *
 ...
 *
 * Recompute. The backward needs lambda_t and eta_t at every step and the
 * forward stores neither. The forward instead checkpoints both every
 * KLA_CHUNK steps into [B,M,NCK,S]; the backward walks the chunks in reverse,
 * replays KLA_CHUNK forward steps from each checkpoint into two thread-private
 * arrays, then runs the adjoint back down over them. lambda could be recovered
 * by inverting the leaf instead, but eta cannot: eta_{t-1} = (eta_t - r_t)/alpha_t
 * divides by a gain that is small exactly when the filter is forgetting, so the
 * error would blow up geometrically along the reverse walk.
 *
